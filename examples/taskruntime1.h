...
typedef struct parallel_worker_context_extension {
	bool is_recyclable;
	task* continuation_task;
	task* current_executable_task;
} parallel_worker_context_extension;

inline worker_context* make_context_with_extension() {
	worker_context* c = make_context();
	parallel_worker_context_extension* e = new parallel_worker_context_extension();
	e->current_executable_task = e->continuation_task = nullptr;
	c->extension = e;
	return c;
}

inline parallel_worker_context_extension* fetch_current_worker_context_extension() {
	return (parallel_worker_context_extension*)worker_contexts[std::this_thread::get_id()]->extension;
}
...
public:
	...
	static task* self() {
		return fetch_current_worker_context_extension()->current_executable_task;
	};

protected:
	static void recycle() {
		fetch_current_worker_context_extension()->is_recyclable = true;
	}
...
inline task& allocate_continuation_proxy::allocate(const size_t size) const {
	task& t = *((task*)this);
	fetch_current_worker_context_extension()->continuation_task = t.continuation();
...
inline task& allocate_child_proxy::allocate(const size_t size) const {
	task& t = *((task*)this);
	fetch_current_worker_context_extension()->continuation_task = &t;
	return *((task*)::operator new(size));
}

inline void custom_schedule(void* v) {
	parallel_worker_context_extension* cx = fetch_current_worker_context_extension();
...
inline void spawn(task& t) {
	spawn(std::this_thread::get_id(), (void*)&t);
}

inline void init_pool() {
	init_pool(custom_schedule, make_context_with_extension);
}
